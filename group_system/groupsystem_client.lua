...
playerGroupWindow.member_list_destroygroup_reclick_window = guiCreateWindow(0.25, 0.2, 0.5, 0.6, "Destory Group", true)
...
playerGroupWindow.member_list_destroygroup_reclick_text = guiCreateLabel(0.05, 0.1, 0.95, 0.3, "Do you realy want to destroy this Group ?", true, playerGroupWindow.member_list_destroygroup_reclick_window)
...
function openPlayerGroupWindow()
  if not getElementData(getLocalPlayer(), "gang") == "logedin" then
    return
  end
...
function openMemberGroupWindow()
  if getElementData(getLocalPlayer(), "gang") == "None" then
    return
  end
...
function refreshGangMemberTable2()
  ...
  for i, gangmember in ipairs(getElementData(getRootElement(), "gangmemberlist_" .. getElementData(getLocalPlayer(), "gang"))) do
    ...
    if gangmember[3] then
      ...
      guiGridListSetItemText(playerGroupWindow.member_list, row, playerGroupWindow.member_list_movestate, (not isPedInVehicle(player) or not "Vehicle") and "Walking", false, false)
    else
...
function playerGroupSystemButton(button, state)
  if not button == "mouse1" and not state == "down" then
    return
  end
...
function isSpamTimer2()
  if isTimer(antiSpamTimer2) then
    do
      ...
      outputChatBox("You have to wait " .. math.floor(t / 1000 + 1) .. " sec. to open the Group Panel", 255, 255, 0, true)
      return true
    end
  else
    return false
